...
class CaseService:
    ...
    def submit_case(self, case_id: int, received_on: date) -> Case:
        ...
        self._deadline_engine.calculate(received_on)
        return self._repository.submit(case_id, case.status, received_on)
    ...
    def record_extension(self, case_id: int, notified_on: date) -> Case:
        ...
        schedule = self._deadline_engine.calculate(date.fromisoformat(case.received_on))
