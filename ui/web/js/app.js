import { createCaseWorkflow } from "./case_workflow.js";
...
const caseWorkflow = createCaseWorkflow({
  setStatus,
  clearNode,
  makeButton,
  localDateString,
...
function analyzeArtifact(artifactId) {
  ...
  backend.analyzeArtifact(selectedInvestigationId, artifactId, (response) => {
    if (response?.ok) {
      ...
      setStatus(count
        ? `${count} deterministic evidence item(s) extracted.`
        : "Artifact already fully analyzed or no supported findings found.");
...
function renderInvestigationDetail(detail) {
  ...
  investigationDetailTitleNode.textContent = detail.investigation.title
...
function renderState(state) {
  milestoneNode.textContent = state.milestone ?? "M22";
...
function connectBackend() {
  ...
  new QWebChannel(qt.webChannelTransport, (channel) => {
    ...
    caseWorkflow.setBackend(backend);
    ...
    backend.researchCompleted.connect((investigationId, _artifactId, result) => {
      ...
      setStatus(count
        ? `${count} research evidence item(s) recorded.`
        : "Research completed with no new evidence.");
    ...
    backend.modelResearchCompleted.connect((investigationId, evidenceId, result) => {
      ...
      const count = result?.createdCount ?? 0;
      setStatus(count
        ? `${count} reviewed research evidence item(s) recorded from Evidence #${evidenceId}.`
        : `Reviewed research for Evidence #${evidenceId} completed with no new evidence.`);
      if (selectedInvestigationId === investigationId) loadInvestigation(investigationId);
    });
    ...
    backend.modelAnalysisCompleted.connect((investigationId, result) => {
      ...
      setStatus(count
        ? `${count} model proposal(s) awaiting review.`
        : "Model analysis returned no proposals.");
    });
...
nameForm.addEventListener("submit", (event) => {
  ...
  backend.setDisplayName(displayNameNode.value, (response) => {
    handleMutation(response, "Profile saved locally.");
  });
});
...
artifactForm.addEventListener("submit", (event) => {
  event.preventDefault();
  if (!backend || !selectedInvestigationId) return;
  backend.importTextArtifact(
    selectedInvestigationId,
    artifactKindNode.value,
    "TRIGGER",
    artifactTextNode.value,
    (response) => {
